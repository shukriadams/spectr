    ...
    glob(modelPath, function(er, models){
        ...
        for (var i = 0 ; i < models.length ; i ++){
            ...
            try {
                ...
                return callback('Error parsing JSON from file' + model);
            }
        }
...
Spectr.prototype.resolve = function(callback){
    ...
    this.load(function(err){
        ...
        if (this._engine)
            this._engine.registerPartials(function(err){
                callback(err);
            });
        else
            callback();
        ...
        self._loadStatic(self.options.models.static, false, function(err){
            ...
            self._loadStatic(self.options.models.pages, true, function(err){
                ...
                for (var modelName in self._models)
                    self._models[modelName] = self._processModel(self._models[modelName]);
